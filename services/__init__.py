...
def extract_invite_code(link: str) -> str | None:
    ...
    Aceita:
      ...
      - discord.gg/abc123 (sem protocolo)
    """
    link = (link or "").strip()
    if not link:
        return None
...
async def check_invite(
    ...
    Retorna uma tupla (ok, motivo, info):
      - ok=True  → link válido E permanente. info tem nome/ícone do servidor.
      - ok=False → motivo explica por que falhou ("inválido", "temporário", etc.)

    info = {"name": ..., "icon_url": ...} quando ok=True
    """
    code = extract_invite_code(link)
    if code is None:
        return False, "O link não parece ser um convite do Discord válido.", None

    # Normaliza pra URL canônica
    ...
    # Convite permanente = max_age 0 (nunca expira)
    ...
    # Confere se já expirou (segurança extra)
